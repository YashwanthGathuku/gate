---
name: run
description: Run a coding task through Gate's protected verification and exact-thread retry loop. Use when the user asks to gate, verify, or safely execute work in a repository. Do not use for ordinary test-only requests or to grade an already-running Codex task.
---

# Run Through Gate

Start a separate Gate-controlled child Codex session for the requested coding
...
3. Do not edit the target repository, run the coding task, or create project
   configuration before launching Gate.
...
6. Relay `FINAL`, `AUDIT_LOG`, and `AUDIT_ROOT` exactly. Do not reinterpret,
   soften, or replace Gate's verdict.
...
Gate-controlled child; it does not retroactively secure the current task.
