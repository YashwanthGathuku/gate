#!/usr/bin/env python3
"""Deterministic adapter between Codex plugin skills and TheUstad."""
...
def ensure_supported_platform(*, os_name: str | None = None) -> None:
    ...
    if current == "nt":
        raise PluginError(
            "TheUstad requires Linux, macOS, or WSL 2; "
...
def default_state_home(
    *,
    ...
    if values.get("THEUSTAD_STATE_HOME"):
        ...
        root = user_home / ".local" / "state" / "theustad"
...
def _validate_external_state_home(repo: Path, state_home: Path) -> None:
    if state_home == repo or state_home.is_relative_to(repo):
        raise PluginError("TheUstad state must remain outside the target repository")
...
def doctor(
    ...
    *,
    ...
    if sys.version_info < (3, 10):
        raise PluginError("TheUstad requires Python 3.10 or newer")
...
def build_theustad_argv(
    *,
    ...
    if protect_add:
        argv.extend(["--protect-add", *protect_add])
    return argv
...
def run_theustad(
    ...
    argv = build_theustad_argv(
        ...
        protect_add=[item for group in args.protect_add for item in group],
    )
    print(f"THEUSTAD_PLUGIN_STATE {state_dir}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="theustad-plugin",
        description="Run the bundled TheUstad runtime from a Codex plugin.",
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    doctor_parser = subparsers.add_parser("doctor", help="check TheUstad prerequisites")
    ...
    run_parser = subparsers.add_parser("run", help="run a TheUstad-controlled Codex task")
    ...
    audit_parser = subparsers.add_parser("audit", help="verify a TheUstad audit chain")
...
def _run_doctor(args: argparse.Namespace) -> int:
    ...
    print(f"THEUSTAD_DOCTOR_REPO {report.repo}")
    print(f"THEUSTAD_DOCTOR_PYTHON {report.python} ({report.python_version})")
    print(f"THEUSTAD_DOCTOR_CODEX {report.codex} ({report.codex_version})")
    print(f"THEUSTAD_DOCTOR_VERIFIER {report.verifier}")
    print(f"THEUSTAD_DOCTOR_STATE_HOME {report.state_home}")
    print("THEUSTAD_DOCTOR_OK")
    return 0
